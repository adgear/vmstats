...
handle_info({timeout, TimerRef, ?TIMER_MSG}, #state {
    ...
    lists:map(fun ({SchedulerId, ShedulerUtil}) ->
        ...
        log_gauge([BaseKey, <<"scheduler_utilization.">>, SchedulerIdBin], ShedulerUtil, 1.00)
    end, ShedulerUtils),

  	% active_tasks
  	lists:foldl(fun(ActiveTasks, SchedulerId) ->
          SchedulerIdBin = integer_to_binary(SchedulerId),
  		log_gauge([BaseKey, <<"active_tasks.">>, SchedulerIdBin], ActiveTasks, 1.00),
  		SchedulerId + 1
  	...
  	lists:foldl(fun(RunQueueLengths, SchedulerId) ->
          SchedulerIdBin = integer_to_binary(SchedulerId),
  		log_gauge([BaseKey, <<"run_queue_lengths.">>, SchedulerIdBin], RunQueueLengths, 1.00),
  		SchedulerId + 1
